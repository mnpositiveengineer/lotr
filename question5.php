		...
		if ($result = $connection -> query("SELECT * FROM users WHERE id='$id';")) 
		{
			...
			if ($isUserInDatabase == 1) 
			{
				if((isset($_POST['answer']))){
					...
					$connection -> query("UPDATE users SET question = 5 WHERE id='$id';");
					$connection -> query("INSERT INTO user_question_hero (user_id, question_id, hero_id) VALUES ('$id', 4, '$question_answer');");
				}
				$result -> close();
				...
				{
					$result -> close();
					$result_question = $connection -> query("SELECT * FROM questions WHERE id=5");
